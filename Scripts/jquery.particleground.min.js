...
!(function (a, b) {
  ...
  function d (d, g) {
    function h () {
      if (y) {
        (r = b.createElement ('canvas')), (r.className =
          'pg-canvas'), (r.style.display = 'block'), d.insertBefore (
          r,
          d.firstChild
        ), (s = r.getContext ('2d')), i ();
        for (
        ...
        a.addEventListener (
          ...
          a.addEventListener (
            'deviceorientation',
            function () {
              (F = Math.min (Math.max (-event.beta, -30), 30)), (E = Math.min (
    ...
    function i () {
      (r.width = d.offsetWidth), (r.height = d.offsetHeight), (s.fillStyle =
        g.dotColor), (s.strokeStyle = g.lineColor), (s.lineWidth = g.lineWidth);
    }
    function j () {
      if (y) {
        (u = a.innerWidth), (v = a.innerHeight), s.clearRect (
          0,
          0,
          r.width,
          r.height
    ...
    function n () {
      switch ((this.stackPos, (this.active = !0), (this.layer = Math.ceil (
        3 * Math.random ()
        ...
        case 'left':
          this.speed.x = +(-g.maxSpeedX +
            Math.random () * g.maxSpeedX -
            g.minSpeedX).toFixed (2);
          break;
        case 'right':
          ...
          break;
        default:
          (this.speed.x = +(-g.maxSpeedX / 2 +
            Math.random () * g.maxSpeedX).toFixed (2)), (this.speed.x += this
            .speed.x > 0
            ? g.minSpeedX
            : -g.minSpeedX);
      }
      switch (g.directionY) {
        case 'up':
          this.speed.y = +(-g.maxSpeedY +
            Math.random () * g.maxSpeedY -
            g.minSpeedY).toFixed (2);
          break;
        case 'down':
          ...
          break;
        default:
          (this.speed.y = +(-g.maxSpeedY / 2 +
            Math.random () * g.maxSpeedY).toFixed (2)), (this.speed.x += this
            .speed.y > 0
            ? g.minSpeedY
            : -g.minSpeedY);
      }
    ...
    function p () {
      console.log ('destroy'), r.parentNode.removeChild (r), q (
        'onDestroy'
    ...
    return (g = c ({}, a[e].defaults, g)), (n.prototype.draw = function () {
      s.beginPath (), s.arc (
        ...
        2 * Math.PI,
        !0
      ), s.closePath (), s.fill (), s.beginPath ();
      ...
      if (g.parallax) {
        ...
        (this.parallaxTargX =
          (w - u / 2) /
          (g.parallaxMultiplier * this.layer)), (this.parallaxOffsetX +=
          (this.parallaxTargX - this.parallaxOffsetX) /
          10), (this.parallaxTargY =
          (x - v / 2) /
          (g.parallaxMultiplier * this.layer)), (this.parallaxOffsetY +=
